...
data = {"weight1/0":w[1][0], "weight1/1": w[1][1], "weight2/0": w[2][0], "weight2/1": w[2][1], "weight3":w[3], "weight4": w[4], "etha": read_json("vectors_new.json", "etha"), "running_index": read_json("vectors_new.json", "running_time_training")}
...
for i in range(len(x)):
    ...
    neuron4.update([neuron1.y, neuron2.y], [w[1][1], w[2][1]])
    neuron5.update([neuron3.y, neuron4.y], [w[3], w[4]])
    new_y.append(neuron5.y*math.pow(10,8))
...
data = read_json("./../accuracy.json", "linear_regression")
data.update({"sigmoid_regression": {"photosynthetic_activity_calc":new_y}})
write_json("./../accuracy.json", data)
print("finished\n")
